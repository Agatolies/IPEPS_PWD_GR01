...
export class DocumentService extends ApiService {
  ...
  public create(payload: DocumentCreatePayload): Observable<Document> {
    return this.post(ApiUriEnum.DOCUMENT_CREATE, payload)
      .pipe(map((response: ApiResponse) =>
          (response.result) ? DocumentHelper.fromDTO(response.data) : DocumentHelper.getEmpty()),
  ...
  public update(payload: DocumentUpdatePayload): Observable<Document> {
    return this.put(ApiUriEnum.DOCUMENT_UPDATE, payload).pipe(map((response: ApiResponse) =>
      (response.result) ? DocumentHelper.fromDTO(response.data) : DocumentHelper.getEmpty()));
  }

  public getDetail(id: string): Observable<Document> {
    return this.get(`${ApiUriEnum.DOCUMENT_DETAIL}/${id}`)
      .pipe(map((response: ApiResponse) =>
      (response.result) ? DocumentHelper.fromDTO(response.data) :
        DocumentHelper.getEmpty()),
        tap((document:Document) =>
        this.details$.next(document)))
      ;
  }
  ...
  public getList(): Observable<Document[]> {
    return this.get(`${ApiUriEnum.DOCUMENT_LIST}`).pipe(
      map((response: ApiResponse) =>
        (response.result) ?
          (response.data as Documentdto[]).map((dto: Documentdto) => DocumentHelper.fromDTO(dto)) :
          []),
      tap((list: Document[]) => {
        this.list$.next(list)
      }));
